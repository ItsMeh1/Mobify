...
async function createGroup(name, desc) {
  ...
  if (!cleanName) return showToast('Group needs a name.');
...
async function loadGroups() {
  ...
  groupsCache.sort((a, b) => (b.members?.length || 0) - (a.members?.length || 0));
    ...
    card.innerHTML = `
      <div class="group-top">
        ...
        <div class="group-meta">
          <div class="group-name">${safeText(g.name)}</div>
      ...
      <div class="group-actions">
        <button class="tool-btn accent" onclick="window.openGroup('${g.id}')">
          Open
        </button>

        <button class="tool-btn ${isMember ? 'danger' : 'success'}"
          onclick="window.toggleGroupJoin('${g.id}')">
          ${isMember ? 'Leave' : 'Join'}
        </button>
      </div>
    `;
...
window.openGroup = async (gid) => {
  ...
  const body = document.getElementById('groupViewContent');

  const isOwnerGroup = g.ownerId === currentUid();

  body.innerHTML = `
    <div class="group-view-header">
      <div class="group-view-icon">
        ${safeText((g.name || 'G')[0])}
      </div>

      <div>
        <h2 style="margin:0;">${safeText(g.name)}</h2>
        <div class="tiny">${g.members?.length || 0} members</div>
      </div>
    </div>

    <div class="group-view-description">
      ${safeText(g.description || 'No description')}
    </div>

    <div class="glass" style="padding:16px; border-radius:16px; margin-top:18px;">
      <div class="tiny">Owner</div>

      <div style="display:flex; align-items:center; gap:10px; margin-top:10px;">
        <img
          src="${g.ownerPfp || fallbackPfp(g.ownerName, g.ownerId)}"
          style="width:42px; height:42px; border-radius:50%; object-fit:cover;"
        >

        <div>
          <div style="font-weight:800;">
            ${safeText(g.ownerName || 'Unknown')}
          </div>

          <div class="tiny">
            @${safeText((g.ownerName || 'user').toLowerCase().replace(/\s/g, ''))}
          </div>
        </div>
      </div>
    </div>
      ...
      isOwnerGroup
        ? `
          <button
            ...
            onclick="window.deleteGroup('${gid}')"
          >
            Delete Group
          </button>
        `
...
window.deleteGroup = async (gid) => {
  ...
  if (g.ownerId !== currentUid()) {
    return showToast('Only the owner can delete this group.');
  }
...
async function addCurrentGroupMember() {
  ...
  if (typeof refreshGroupList === 'function') await refreshGroupList();
...
document.addEventListener('click', async (e) => {
  ...
  if (btn.id === 'openCreateGroupBtn') {
    e.preventDefault();
    if (typeof window.openCreateGroup === 'function') window.openCreateGroup();
    return;
  }
...
window.addEventListener('load', async () => {
  const list = getGroupListElement();
